...
export async function runSpeedMonitor(env, analyzePage) {
  ...
  for (const url of batch) {
    try {
      ...
      const record = normalizeAnalysis(data, url);
      results.push(record);
      await env.CURATOR_SPEED_RECORDS.put(PAGE_PREFIX + record.path, JSON.stringify(record));
    } catch (error) {
      const record = { path: toPath(url), url, scannedAt: new Date().toISOString(), ok: false, error: error?.message || String(error), responseTimeMs: null, actionableCount: 0, highCount: 0, mediumCount: 0, verdict: 'error' };
      results.push(record);
      await env.CURATOR_SPEED_RECORDS.put(PAGE_PREFIX + record.path, JSON.stringify(record));
    }
  }
  ...
  const snapshot = await buildSnapshot(env, urls.length, nextCycle);
...
async function buildSnapshot(env, discoveredPageCount, cycle) {
  ...
  return {
    ...
    healthyPageCount: pages.filter(p => p.verdict === 'healthy').length,
    attentionPageCount: attention.length,
    opportunityPageCount: opportunities.length,
    errorPageCount: pages.filter(p => p.ok === false).length,
    averageResponseTimeMs: avg,
    medianResponseTimeMs: p50,
...
function normalizeAnalysis(data, requestedUrl) {
  ...
  return {
    ...
    ok: true,
    responseTimeMs: Number(data.metrics?.responseTimeMs || 0),
    htmlBytes: Number(data.metrics?.htmlBytes || 0),
    imageCount: Number(data.metrics?.imageCount || 0),
    scriptCount: Number(data.metrics?.scriptCount || 0),
    stylesheetCount: Number(data.metrics?.stylesheetCount || 0),
    actionableCount: Number(data.verdict?.actionableCount || 0),
    highCount: Number(data.verdict?.highCount || 0),
    mediumCount: Number(data.verdict?.mediumCount || 0),
    verdict: data.verdict?.status || 'healthy',
